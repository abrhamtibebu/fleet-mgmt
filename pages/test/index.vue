...
const { vehicleList, loading, getVehicles, activeVehicles, serviceDueVehicles, calculateServiceDue, getVehicleReport} = useVehicles()
const { fuelCards, getFuelCards, lowBalanceCards, fuelRecords, getFuelRecords, getFuelReports,fuelReports } = useFuel()
...
from.value = startOfYear.toISOString().split('T')[0]
to.value = today.toISOString().split('T')[0]

const reloadReports = async () => {
  await Promise.all([
    getFuelReports(from.value, to.value),
    getVehicleReport(from.value, to.value)
  ])
  initializeCharts()
}
// KPI/Stats
...
const getMonthlyFuelConsumption = () => {
  ...
  return months.map((month, index) => {
    const monthRecords = fuelReports.value.filter(record => {
